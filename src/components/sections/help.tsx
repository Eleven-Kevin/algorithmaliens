        ...
        <div className="max-w-4xl mx-auto space-y-12">
          {faqs.map((category, catIndex) => (
            <div key={catIndex}>
              ...
              <Accordion type="single" collapsible className="w-full bg-card p-6 rounded-lg">
                {category.items.map((faq, index) => (
                  <AccordionItem key={index} value={`item-${catIndex}-${index}`} className="border-b-border/50 last:border-b-0">
